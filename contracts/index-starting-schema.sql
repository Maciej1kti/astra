...
CREATE INDEX IF NOT EXISTS documents_schedule_end ON documents(project_id,json_extract(metadata_json,'$.schedule.end')) WHERE entity_type='card';
CREATE INDEX IF NOT EXISTS documents_milestone_due ON documents(project_id,json_extract(metadata_json,'$.due.date')) WHERE entity_type='milestone';
