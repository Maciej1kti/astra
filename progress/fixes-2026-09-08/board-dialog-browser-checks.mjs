...
} from "../../scripts/settings-dialog-regression.mjs";
import { checkFocusOrderKeyboard } from "../../scripts/focus-order-regression.mjs";
...
const evidence = join(import.meta.dirname, "board-dialog-browser");
...
const cli = (...args) => {
  ...
  try {
    output = execFileSync(
      join(root, "target/release/projectctl"),
...
async function check(id, name, run) {
  ...
  await writeFile(
    resultsFile,
    JSON.stringify({ results, errors, browser: browser.version() }, null, 2),
  );
}
...
async function settings() {
  ...
  await expect(dialog.getByLabel("Timezone", { exact: true })).toBeEnabled();
  return dialog;
}
...
try {
  ...
  await page.getByRole("button", { name: "Request access" }).waitFor();
  await page
  ...
  await check(
    ...
    async () => {
      ...
      await expect(page.locator(".astra-board .wx-theme")).not.toHaveAttribute(
        "style",
      );
      ...
      assert.deepEqual(await page.evaluate(() => window.boardDialogCsp), []);
      return { csp, singleCardAction: true, metadataMeaningsRetained: true };
    },
  );
  ...
  await check(
    ...
    async () => {
      ...
      await dialog.getByText("Pending command:", { exact: false }).waitFor();
      ...
      await dialog
        ...
        .click();
      await dialog
        .getByRole("button", { name: "Discard settings draft", exact: true })
        .click();
  ...
  await check(
    ...
    async () => {
      ...
      await dialog
        ...
        .click();
      await expect(
        dialog.getByRole("button", { name: "Close focus order", exact: true }),
      ).toBeFocused();
      await dialog
        .getByRole("button", { name: "Close focus order", exact: true })
        .press("Control+Enter");
