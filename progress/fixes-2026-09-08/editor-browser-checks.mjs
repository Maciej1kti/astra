...
async function main() {
  ...
  const evidenceDir = resolve(root, process.env.ASTRA_EVIDENCE_DIR ?? "progress/fixes-2026-09-08/editor-browser");
  const config = JSON.parse(await readFile(join(runtimeDir, "connection.json"), "utf8"));
  const cli = (...args) => {
    let output;
    try { output = execFileSync(join(root, "target/release/projectctl"), ["--socket", config.socket, ...args], { encoding: "utf8", stdio: ["ignore", "pipe", "pipe"] }); }
