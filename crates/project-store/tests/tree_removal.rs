...
fn deletion_removes_only_the_inventoried_project_tree() {
    ...
    fs::write(root.join(".project/project.md"), "project source").unwrap();
    fs::write(root.join(".project/cards/card.md"), "card source").unwrap();
...
fn changed_parent_is_rejected_even_if_file_identity_and_bytes_match() {
    ...
    fs::rename(
        root.join("old-cards/card.md"),
        root.join(".project/cards/card.md"),
    )
    .unwrap();
    assert!(remove_step(&root, &observed, 0).is_err());
    assert!(root.join(".project/cards/card.md").exists());
}
