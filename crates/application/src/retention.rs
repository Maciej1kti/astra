...
impl Journal {
    ...
    pub fn retain(&self, now: i64) -> Result<Value, AppError> {
        ...
        tx.execute_batch(
            "CREATE TEMP TABLE IF NOT EXISTS retention_commands(epoch TEXT,
    request_id TEXT,
    PRIMARY KEY(epoch,
    request_id)); DELETE
FROM retention_commands;",
        )?;
        tx.execute(
            "INSERT INTO retention_commands SELECT c.epoch,
    c.request_id
FROM commands c
WHERE c.expires_at<?1
AND c.state IN ('committed',
    'rejected')
AND NOT EXISTS(SELECT 1
FROM write_intents w
WHERE w.epoch=c.epoch
AND w.request_id=c.request_id
AND w.resolved=0)
AND NOT EXISTS(SELECT 1
...
WHERE j.epoch=c.epoch
AND j.request_id=c.request_id
AND j.state!='done')
...
WHERE json_extract(p.plan_json,
    '$.expires_at')<?1
AND NOT EXISTS(SELECT 1
FROM workflow_jobs j
...
LIMIT 100)",
        ...
        let bytes: i64 = tx.query_row(
            "SELECT COALESCE(sum(COALESCE(length(before_bytes),
    0)+COALESCE(length(after_bytes),
    0)),
    0)
FROM history",
            [],
        ...
        let rows = {
            let mut statement = tx.prepare(
                "SELECT h.id,
    h.recorded_at,
    COALESCE(length(h.before_bytes),
    0)+COALESCE(length(h.after_bytes),
    0)
FROM history h
WHERE h.pinned=0
AND NOT EXISTS(SELECT 1
FROM commands c
WHERE c.epoch=h.epoch
AND c.request_id=h.request_id
AND (c.expires_at>=?1
OR c.state NOT IN ('committed',
    'rejected')))
...
FROM pairings
WHERE expires_at<?1
AND (claim_grace_until IS NULL
OR claim_grace_until<?1)
LIMIT 500)",
...
AND NOT EXISTS(SELECT 1
FROM pairings p
WHERE p.last_issued_session_id=s.id)
LIMIT 500)",
            [instant(now - 7 * 86_400_000)],
        )?;
        tx.execute("INSERT INTO meta(key,
    value)
VALUES ('history_retention',
    ?1)
ON CONFLICT (key) DO UPDATE
SET value=excluded.value",params![json!({"checked_at":instant(now),"history_bytes":total,"history_removed":removed,"commands_removed":commands}).to_string()])?;
        tx.commit()?;
