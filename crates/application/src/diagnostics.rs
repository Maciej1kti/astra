...
impl Engine {
    pub fn diagnostics(&self) -> Result<Value, AppError> {
        ...
        if workspace.is_none() {
            ...
            warnings.push(json!({
                "code": if missing {"WORKSPACE_MISSING"} else {"WORKSPACE_INVALID"},
                "message": "Project writes are unavailable. Stop the server and repair the workspace file from your own known-good source; the registry has not been recreated.",
            }));
        }
...
WHERE state IN ('prepared',
    ...
    'needs_review')",
            ...
            .is_some_and(|floor| now_millis() < floor - 300_000)
        {
            warnings.push(json!({
                "code": "CLOCK_ROLLBACK",
                "message": "The host clock moved backwards. Correct the clock before issuing new mutations; known command results remain queryable.",
            }));
        }
        let (entries, bytes): (i64, i64) = db.query_row(
            "SELECT count(*),
    COALESCE(sum(COALESCE(length(before_bytes),
...
LIMIT 50",
        ...
        let jobs = statement
            .query_map([], |row| {
                ...
                Ok(json!({ "id": id, "state": state, "project_id": project_id }))
            })?
        ...
        if count > 0 {
            warnings.push(json!({
                "code": "SOURCE_DIAGNOSTICS",
                "message": "Inspect the listed source paths. Validate documents and use an explicit normalization plan where required; healthy project files remain editable.",
            }));
        }
        Ok(json!({
            "instance_id": instance,
            "state": if warnings.is_empty(){"ready"}else{"degraded"},
            "invalid_documents": count,
            "pending_commands": pending,
            "index_state": if count>0 || workspace.is_none(){"degraded"}else if building{"building"}else{"ready"},
            "warnings": warnings,
            "issues": issues,
            "jobs": jobs,
            "history": {
