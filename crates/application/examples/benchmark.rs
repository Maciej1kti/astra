...
use project_application::{Mutation, Query, engine::Engine, now_millis};
...
fn statistics(mut samples: Vec<f64>) -> Value {
    ...
    json!({
        ...
        "p99_ms": samples[(n*99/100).min(n-1)],
    })
}
fn main() {
    ...
    for n in 0..projects {
        ...
        for u in 0..updates {
            ...
            let meta = json!({
                ...
                "summary": format!("Synthetic report {u}"),
                "author": {
                    "kind": "human",
                    "label": "Benchmark",
                },
    ...
    for n in 0..220 {
        ...
        let reply = engine
            .mutate(Mutation {
                ...
                epoch: engine.command_epoch().to_owned(),
                expected: if create {
                    None
                } else {
    ...
    println!("{}",serde_json::to_string_pretty(&json!({
        ...
        "attention": statistics(attention),
