...
export type TagIssue = { project_id?: string; card_id?: string; message: string };
...
export type TagCatalog = { version: string; tags: CatalogTag[]; complete: boolean; issues: TagIssue[] };
...
export function catalogNameError(name: string, existing: readonly string[] = []): string {
  if (!name.trim()) return "Enter a tag name.";
  if (name !== name.trim()) return "Remove spaces at the start and end of the name.";
  if ([...name].length > 48) return "A tag can contain up to 48 characters.";
  if (existing.includes(name)) return "This exact name is already in the catalog.";
  return "";
}
...
export function canFinishTagChange(catalog: TagCatalog | null, preview: TagPreview | null, results: TagChangeResult[]): boolean {
  return !!catalog && !!preview && catalog.complete && preview.complete &&
