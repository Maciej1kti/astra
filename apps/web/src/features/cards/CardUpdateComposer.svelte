<script lang="ts">
  import { createUpdate } from "../../lib/api/resources";
  ...
  let error = $state("");
  let message = $state("");
  const operation = commandOperation(() => !disabled);
  $effect(() => {
  ...
  async function post() {
    ...
    try {
      operation.prepare(
        createUpdate(project, cardUpdatePayload(cardId, draft)),
      );
      await transmit();
    } catch (cause) {
  ...
  async function transmit() {
    if (!operation.pending || busy || disabled) return;
    error = "";
    try {
