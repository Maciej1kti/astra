...
test("date-only moves preserve duration across leap days, month/year changes and DST", () => {
  for (const [from, delta, to] of [
    ...
    assert.equal(shiftDate(from, delta), to);
  for (let delta = -400; delta <= 400; delta++) {
    const result = shiftedSchedule(
...
test("resize changes only its boundary and never produces a reversed range", () => {
  ...
  assert.deepEqual(shiftedSchedule(range, 100, "start"), {
    start: range.end,
    end: range.end,
  });
  assert.deepEqual(shiftedSchedule(range, -100, "end"), {
    start: range.start,
    end: range.start,
  });
  assert.deepEqual(shiftedSchedule(range, 2, "end"), {
    start: range.start,
    end: "2026-09-14",
  });
});
