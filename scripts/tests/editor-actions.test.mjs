...
import { editorCompletion, canUndoDraft } from "../../apps/web/src/lib/editor-actions.ts";
import { addTag, tagValidation, matchingTags } from "../../apps/web/src/lib/tags.ts";

test("focus and read receipt commits preserve the editor session, including status recovery", () => {
  assert.equal(editorCompletion({ path: "/api/v1/workspace/focus" }), "focus");
  assert.equal(editorCompletion({ path: "/api/v1/workspace/read-receipts" }), "read");
  assert.equal(editorCompletion({ path: "/api/v1/projects/p/cards/c" }), "resource");
});
...
test("editing unrelated fields does not split, trim or normalize source tags", () => {
  const source = ["Design, research", " polskie znaki ąę ", "Cafe\u0301", "Café", "QA"];
  ...
  assert.deepEqual(source, ["Design, research", " polskie znaki ąę ", "Cafe\u0301", "Café", "QA"]);
});
...
test("source suggestions preserve exact spelling and omit selected tags", () => {
  assert.deepEqual(matchingTags(["UI", "UX", "UI", " ui ", "Planning"], ["UI"], "u"), [" ui ", "UX"]);
