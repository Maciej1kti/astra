...
import {
  ...
  destinationTag,
  canFinishTagChange,
} from "../../apps/web/src/features/tags/tag-management.ts";
...
test("finishing a tag change requires complete source coverage and every reviewed card saved", () => {
  assert.equal(
    canFinishTagChange(catalog, preview, [{ state: "saved" }]),
    true,
  );
  ...
  assert.equal(
    canFinishTagChange({ ...catalog, complete: false }, preview, []),
    false,
  );
  assert.equal(
    canFinishTagChange(catalog, { ...preview, complete: false }, []),
    false,
  );
  assert.equal(
    canFinishTagChange(
      { ...catalog, tags: [{ name: "Old", usage: 1 }] },
      preview,
      [],
    ),
    false,
...
test("catalog replacements retain managed exact names and do not silently promote all observed labels", () => {
  ...
  assert.deepEqual(
    catalogNames({ ...catalog, tags: [{ name: " existing ", managed: true }] }),
    [" existing "],
  );
});
