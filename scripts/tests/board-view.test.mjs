...
import { readBoardView, writeBoardView } from '../../apps/web/src/lib/board-view.ts';

test('board preferences isolate projects and never persist card data or cursors', () => {
  const data = new Map();
  const storage = {getItem: key => data.get(key) ?? null, setItem: (key,value) => data.set(key,value)};
  writeBoardView('project-a',{horizontal:240,vertical:{review:180},collapsed:{done:true},title:'private',cursor:'expired'},storage);
...
test('untrusted or unavailable browser storage cannot break the board', () => {
  for (const value of ['null','[]','broken','{"horizontal":-1,"vertical":{"review":"bad"},"collapsed":{"done":"true"}}']) {
  ...
  assert.doesNotThrow(()=>writeBoardView('p',{horizontal:0,vertical:{},collapsed:{}},denied));
});
