...
const evidence = resolve(
  root,
  process.env.ASTRA_EVIDENCE_DIR ?? "test-results/browser/regressions/tags",
);
...
async function preview(source, target) {
  ...
  await manager()
    .getByRole("button", { name: `Rename or merge tag ${source}`, exact: true })
    .click();
...
try {
  ...
  await check(
    ...
    async () => {
      ...
      assert.deepEqual(
        cli("get", `/api/v1/projects/${other}/cards/${card}`).metadata.labels,
        [name],
      );
  ...
  await check(
    ...
    async () => {
      ...
      try {
        ...
        await manager()
          .getByRole("button", { name: "Retry same card command", exact: true })
          .click();
        await expect(
          manager().getByRole("button", {
            name: "Finish catalog change",
    ...
    async () => {
      ...
      await mutate(
        ...
        "/api/v1/workspace/preferences",
        {
          preferences: {
            week_start:
              before.preferences.week_start === "sunday" ? "monday" : "sunday",
          },
        },
        before.version,
      );
  ...
  await check(
    ...
    async () => {
      ...
      await manager()
        ...
        .click();
      await expect(
        manager().getByRole("button", { name: "Keep reviewing", exact: true }),
      ).toBeFocused();
      await manager()
