...
const project = config.projects[0].id, card = config.cards[0];
const base = `/api/v1/projects/${project}`;
const command = join(runtime, "report.json");
for (let index = 0; index < 55; index++) {
  await writeFile(command, JSON.stringify({ kind: "note", summary: `History entry ${String(index + 1).padStart(2, "0")}`,
    target: { type: "card", id: card.id }, author: { kind: "human", label: "Synthetic QA" },
    body: `## Lazy body ${index + 1}\n\nSynthetic history evidence.` }), { mode: 0o600 });
...
try {
  ...
  page.on("request", (request) => {
    ...
    if (request.method() === "GET" && url.pathname.startsWith("/api/")) requests.push(url.pathname + url.search);
  });
  await page.goto(`${config.origin}/?${new URLSearchParams({ view: "list", project })}`);
  ...
  await Promise.all([
    page.waitForResponse((response) => response.url().includes("/views/list?") && new URL(response.url()).searchParams.get("type") === "card"),
    page.getByRole("button", { name: "Refresh", exact: true }).click(),
  ]);
  await expect(page.getByText("Loading resources…", { exact: true })).toHaveCount(0);
  const refresh = requests.slice(start).filter((path) => !path.includes("/events"));
  assert(refresh.some((path) => path.includes("/views/list?") && path.includes("type=card")), JSON.stringify(refresh));
  assert(refresh.every((path) => path === "/api/v1/projects?limit=200" || path.startsWith("/api/v1/projects?") || (path.startsWith("/api/v1/views/list?") && path.includes("type=card"))), JSON.stringify(refresh));
  results.push({ check: "List refresh reads only projects and cards", requests: refresh });
  ...
  assert.equal(requests.filter((path) => path.startsWith(`${base}/updates/`)).length, 0, "Report bodies must stay lazy");
  await history.getByRole("button", { name: "Next updates", exact: true }).click();
  ...
  await expect(history.getByRole("button", { name: "Next updates", exact: true })).toHaveCount(0);
  await history.locator("li > button").first().click();
  await expect(history.getByRole("heading", { name: /^Lazy body / })).toBeVisible();
  await history.getByRole("button", { name: "Previous updates", exact: true }).click();
  await expect(history.locator("li")).toHaveCount(50);
  assert.deepEqual(await history.locator("li > button").allTextContents(), first);
  const activity = requests.filter((path) => path.startsWith(`${base}/updates?`));
  ...
  results.push({ check: "Code health browser suite", error: String(error) });
  process.exitCode = 1;
} finally {
  await browser.close();
