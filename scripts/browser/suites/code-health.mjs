...
try {
  const context = await browser.newContext({
    ...
    storageState: join(runtime, "browser-state.json"),
  ...
  assert(
    refresh.every(
      (path) =>
        ...
        (path.startsWith("/api/v1/views/list?") && path.includes("type=card")),
    ),
    JSON.stringify(refresh),
  );
  results.push({
  ...
  assert.equal(
    requests.slice(start).filter((path) => path.startsWith(`${base}/updates`))
      .length,
    0,
  ...
  results.push({
    check: "Relation search finds a card behind more than 50 matching reports",
  });
  await dialog
