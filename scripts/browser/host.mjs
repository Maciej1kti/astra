...
export async function createHost() {
  ...
  try {
    ...
    daemon.stderr.on("data", (data) => {
      daemonLog = (daemonLog + data).slice(-65536);
    });
...
export async function pair(page, host) {
  ...
  if (await request.isVisible()) {
    ...
    const matching = host
      .cli("pairings")
      .items.filter((entry) => text.includes(entry.challenge));
    assert.equal(
      matching.length,
      1,
      "The browser must match exactly one owner-approved challenge",
    );
...
export async function writeRuntime(host, config) {
  ...
  await writeFile(join(runtime, "connection.json"), JSON.stringify(config), {
    mode: 0o600,
  });
  return runtime;
}
