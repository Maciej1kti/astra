...
export async function seed(host) {
  ...
  const projects = [];
  for (const title of [
    "Studio launch — QA synthetic",
    "Personal lab — QA synthetic",
    "Empty project — QA synthetic",
  ]) {
  ...
  const titles = [
    ...
    "Conflict probe",
    "Archive probe",
    "Keyboard order probe",
    "Undated backlog idea",
    "Polish copy — Zażółć gęślą jaźń 🧪",
    "A long synthetic title exercises wrapping while keeping actions reachable",
  ];
  for (let i = 0; i < 36; i++) {
    const payload = {
      ...
      ...(i % 6 !== 3
        ? {
            schedule: {
              start: `2026-09-${String(7 + (i % 15)).padStart(2, "0")}`,
              end: `2026-09-${String(9 + (i % 15)).padStart(2, "0")}`,
            },
          }
        : {}),
      ...(i % 3 === 0
  ...
  const other = await create(`/api/v1/projects/${projects[1].id}/cards`, {
    title: "Separate project focus probe",
    status: "active",
  });
  for (const [kind, summary] of [
    ["result", "Prototype tested on synthetic fixtures"],
    ["blocker", "Awaiting launch decision"],
    ["decision_needed", "Choose the launch date"],
    ["note", "Audit notes and observations"],
  ]) {
    await create(`${base}/updates`, {
      kind,
      summary,
      author: { kind: "human", label: "Synthetic QA" },
      target: { type: "project", id: projects[0].id },
      body: "Synthetic content. No customer data.",
    });
  }
  return {
    origin: host.origin,
